...
export function OrderFormPage() {
    ...
    const [lastAutoGeneratedOrderNumber, _setLastAutoGeneratedOrderNumber] = useState<string>("")
            ...
            while (!success && attempts < MAX_ATTEMPTS) {
                ...
                try {
                    ...
                    if (isConflict && attempts < MAX_ATTEMPTS) {
                        ...
                        const [newReceiptRes] = await Promise.all([
                        ...
                        // USER REQUEST: Do not overwrite manually entered order numbers on conflict
                        /*
    ...
    const generateNextOrderNumber = async (isPeriodic = false, _force = false) => {
        try {
            ...
            // USER REQUEST: No auto-generar número de pedido, dejar campo vacío para ingreso manual
    ...
    useEffect(() => {
        ...
        // USER REQUEST: Do not calculate consecutive order numbers automatically
        /*
        ...
        if (isEditing) {
            try {
                ...
                setCurrentItem((prev: BrandItem) => ({
                    ...
                    total: 0,
                    quantity: 1,
                    orderNumber: "" // USER REQUEST: Leave empty
                }))
            } catch (error: any) {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <td className="px-2 py-2 border-r border-slate-50 text-center font-mono text-[10px] font-bold text-monchito-purple">
                                                    {!isEditing ? (
                                                        <Input
                                                            className="h-7 w-24 mx-auto text-center font-mono text-[10px] font-bold text-monchito-purple border border-slate-200 rounded-lg px-1 focus-visible:ring-1 focus-visible:ring-monchito-purple outline-none shadow-none"
                                                        ...
                                                        <span className="text-center block font-mono text-[10px] font-bold text-monchito-purple">
                                                            {item.orderNumber || "---"}
                                                        </span>
                                                    )}
                                                </td>
